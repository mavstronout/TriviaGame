module.exports = (message) => {
	...
	if (message.channel.permissionOverwrites.size > 0) {
		...
		if (message.channel.permissionOverwrites.has(message.author.id)) {
			binaryAllow.push(message.channel.permissionOverwrites.get(message.author.id).allow);
		...
		binaryDeny.some((bin) => {
			let reversedBin = (bin >>> 0).toString(2).split('').reverse().join('');
			if (reversedBin.charAt(14) === '1') {
				denied = true;
				return true;
			} else {
				denied = false;
